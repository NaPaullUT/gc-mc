...
def load_official_trainvaltest_split(dataset, testing=False):
    ...
    mat_fact_train = np.zeros(num_users * num_items, dtype=np.float32)
    mat_fact_train[train_idx] = labels[train_idx].astype(np.float32) + 1.
    mat_fact_train = mat_fact_train.reshape(num_users, num_items)
    mat_fact_train = mat_fact_train[~np.all(mat_fact_train == 0, axis=1)]

    mat_fact_val = np.zeros(num_users * num_items, dtype=np.float32)
    mat_fact_val[val_idx] = labels[val_idx].astype(np.float32) + 1.
    mat_fact_val = mat_fact_val.reshape(num_users, num_items)
    mat_fact_val = mat_fact_val[~np.all(mat_fact_val == 0, axis=1)]
...
def load_official_trainvaltest_split_cold_start(dataset, testing=False, n_r=1,n_c=0):
    """
    Loads official train/test split and uses 10% of training samples for validaiton
    For each split computes 1-of-num_classes labels. Also computes training
    adjacency matrix. Assumes flattening happens everywhere in row-major fashion.
    ...
    while i<(len(c_users)):
        if np.count_nonzero(pairs_nonzero[:,0]==c_users[i])<n_r:
            #find a new user
            c_users[i] = np.random.choice(u_nodes_ratings, 1, replace=False)
        else:
            i+=1
    # now c_users is a list of users we will turn into cold start users
    ...
    #this is the excluded part of cold start users that should be put in validation set
    idx_cold_val = np.array([u * num_items + v for u, v in u_pairs])
    idx_nonzero = np.setdiff1d(idx_nonzero,idx_cold_val)
    idx_cold_val = np.setdiff1d(idx_cold_val, idx_cold_start)

    num_train-=idx_cold_start.size
    num_val-=idx_cold_val.size
    ...
    val_idx = np.concatenate([idx_nonzero[0:num_val],idx_cold_val])
    train_idx = np.concatenate([idx_nonzero[num_val:num_train + num_val],idx_cold_start])
    test_idx = idx_nonzero_test

    idx_nonzero = np.concatenate([val_idx, train_idx,test_idx], axis=0)

    pairs_nonzero = np.concatenate([u_pairs,pairs_nonzero_train, pairs_nonzero_test])
